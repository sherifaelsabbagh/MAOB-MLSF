...
# Save once -- identical across every algorithm's script, needed later
# by plot_combined_pr_curves.py
np.save('y_test_active.npy', y_test_active)
...
results = []
for run in range(5):
    model = RandomForestRegressor(n_estimators=400, max_features="sqrt", random_state=run, n_jobs=-1)
...
results_df.to_csv('rf_regression_results.csv', index=False)
...
# Retrain the median-representative run, save its predictions AND plot
closest_idx = (results_df['NEF1%'] - median_nef1).abs().idxmin()
closest_seed = int(results_df.loc[closest_idx, 'run'])
model = RandomForestRegressor(n_estimators=400, max_features="sqrt", random_state=closest_seed, n_jobs=-1)
model.fit(X_train, y_train_pic50)
y_pred = model.predict(X_test)

np.save('RF_median_run_predictions.npy', y_pred)
...
ax.set_title(f'RF regression — median run (seed={closest_seed})')
plt.savefig('RF_regression_PR_curve.png', dpi=200)
