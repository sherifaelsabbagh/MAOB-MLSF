...
for run in range(5):
    model = RandomForestClassifier(n_estimators=400, max_features="sqrt",
                                     class_weight="balanced", random_state=run, n_jobs=-1)
...
results_df.to_csv('rf_classification_mfplec_results.csv', index=False)
...
model = RandomForestClassifier(n_estimators=400, max_features="sqrt",
                                 class_weight="balanced", random_state=closest_seed, n_jobs=-1)
model.fit(X_train, y_train_active)
y_score = model.predict_proba(X_test)[:, 1]

np.save('RF_classification_mfplec_median_predictions.npy', y_score)
...
ax.set_title(f'RF classification (MF+PLEC) — median run (seed={closest_seed})')
plt.savefig('RF_classification_mfplec_PR_curve.png', dpi=200)
