...
# XGB's equivalent to class_weight="balanced": scale_pos_weight = n_negative / n_positive
...
for run in range(5):
    model = XGBClassifier(n_estimators=300, max_depth=6, learning_rate=0.1,
                           ...
                           random_state=run, n_jobs=-1, eval_metric="logloss")
...
results_df.to_csv('xgb_classification_results.csv', index=False)
...
model = XGBClassifier(n_estimators=300, max_depth=6, learning_rate=0.1,
                       subsample=0.8, colsample_bytree=0.8,
                       scale_pos_weight=scale_pos_weight,
                       random_state=closest_seed, n_jobs=-1, eval_metric="logloss")
model.fit(X_train, y_train_active)
y_score = model.predict_proba(X_test)[:, 1]

np.save('XGB_classification_median_predictions.npy', y_score)
...
ax.set_title(f'XGB classification — median run (seed={closest_seed})')
plt.savefig('XGB_classification_PR_curve.png', dpi=200)
