...
results_df.to_csv('xgb_regression_mfplec_results.csv', index=False)
...
model = XGBRegressor(n_estimators=300, max_depth=6, learning_rate=0.1,
                      subsample=0.8, colsample_bytree=0.8,
                      random_state=closest_seed, n_jobs=-1)
model.fit(X_train, y_train_pic50)
y_pred = model.predict(X_test)

np.save('XGB_mfplec_median_predictions.npy', y_pred)
...
ax.set_title(f'XGB regression (MF+PLEC) — median run (seed={closest_seed})')
plt.savefig('XGB_mfplec_PR_curve.png', dpi=200)
