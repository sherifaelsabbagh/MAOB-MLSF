...
Hyperparameter-tuned SVM (Nystroem + SGD approximation) regression for
MAO-B (Section D, v2 dataset, PLEC features).
...
NOTE: exact SVR(kernel='rbf') was tested directly on this project's
full training set (124,092 rows) and timed out (killed at the 4-hour
Slurm wall-clock limit with no output at all), confirming exact SVR is
computationally intractable at this scale. The Nystroem-kernel-
approximation + SGDRegressor pipeline already validated for this
project's baseline SVM sweep is used here for tuning as well.

Search space:
    C (converted to SGD's alpha internally):  0.01-100 (log scale)
    gamma:                                     1e-5 to 1e0 (log scale)
    n_components (Nystroem landmarks):         held FIXED at 1500, not
                                                tuned, to avoid adding a
                                                further memory-variable
                                                search dimension given
                                                this approach already
                                                required 64GB+ memory at
                                                a fixed 1500 in the
                                                baseline sweep.
...
Outputs:
    svm_tuning_history.csv
    svm_tuned_results.csv
    svm_tuned_median_predictions.npy
    SVM_tuned_PR_curve.png
"""
...
pd.DataFrame(tuning_log).to_csv('svm_tuning_history.csv', index=False)
print("Saved svm_tuning_history.csv")
...
results_df.to_csv('svm_tuned_results.csv', index=False)
...
np.save('svm_tuned_median_predictions.npy', y_pred)
...
ax.set_title(f'SVM (tuned) regression — median run (seed={closest_seed})')
plt.savefig('SVM_tuned_PR_curve.png', dpi=200)
print("Saved SVM_tuned_PR_curve.png and svm_tuned_median_predictions.npy")
