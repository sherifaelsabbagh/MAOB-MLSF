"""
svm_classification_tuning.py

Hyperparameter-tuned SVM (Nystroem + SGD approximation) CLASSIFICATION
for MAO-B (Section D, v2 dataset, PLEC features) -- classification
counterpart to svm_tuning.py (regression), on the IDENTICAL train/
test data.

Same rationale as rf_classification_tuning.py: Average Precision (not
MCC) as the CV objective. Nystroem+SGD approximation used for the same
reason established in this project's regression work (exact SVC/SVR
is computationally intractable at this dataset's scale).

Search space (same ranges as svm_tuning.py's regression search):
    C (converted to SGD's alpha internally):  0.01-100 (log scale)
    gamma:                                     1e-5 to 1e0 (log scale)
    n_components: fixed at 1500, not tuned (same rationale as
    svm_tuning.py).
class_weight="balanced" applied throughout (fixed, not tuned).

Outputs:
    svm_classification_mfplec_tuning_history.csv
    svm_classification_mfplec_tuned_results.csv
    svm_classification_mfplec_tuned_median_predictions.npy
    SVM_classification_mfplec_tuned_PR_curve.png
"""
...
def build_svm(params, seed):
    ...
    return Pipeline([
        ...
        ("sgd", SGDClassifier(loss="log_loss", alpha=alpha, class_weight="balanced", random_state=seed)),
    ])
...
pd.DataFrame(tuning_log).to_csv('svm_classification_mfplec_tuning_history.csv', index=False)
print("Saved svm_classification_mfplec_tuning_history.csv")
...
results_df.to_csv('svm_classification_mfplec_tuned_results.csv', index=False)
...
np.save('svm_classification_mfplec_tuned_median_predictions.npy', y_score)
...
ax.set_title(f'SVM (tuned) classification — median run (seed={closest_seed})')
plt.savefig('SVM_classification_mfplec_tuned_PR_curve.png', dpi=200)
print("Saved SVM_classification_mfplec_tuned_PR_curve.png and svm_classification_mfplec_tuned_median_predictions.npy")
