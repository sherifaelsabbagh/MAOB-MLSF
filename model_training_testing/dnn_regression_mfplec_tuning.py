...
Hyperparameter-tuned DNN (scikit-learn MLPRegressor, multi-layer)
regression for MAO-B (Section D, v2 dataset, PLEC features).
...
Search space (informed by a prior manual comparison in this project:
a (1024, 512, 256) architecture UNDERPERFORMED a smaller (256, 128,
64) baseline -- NEF1% 0.122 vs 0.163 on a single-run test -- so this
search brackets sensible sizes around that working baseline, rather
than assuming larger is better):
    n_layers:            2-4
    hidden_layer_size:   {64, 128, 256, 512} (categorical, applied
...
Outputs:
    dnn_regression_mfplec_tuning_history.csv
    dnn_regression_mfplec_tuned_results.csv
    dnn_regression_mfplec_tuned_median_predictions.npy
    DNN_regression_mfplec_tuned_PR_curve.png
"""
...
pd.DataFrame(tuning_log).to_csv('dnn_regression_mfplec_tuning_history.csv', index=False)
print("Saved dnn_regression_mfplec_tuning_history.csv")
...
results_df.to_csv('dnn_regression_mfplec_tuned_results.csv', index=False)
...
np.save('dnn_regression_mfplec_tuned_median_predictions.npy', y_pred)
...
ax.set_title(f'DNN (tuned) regression — median run (seed={closest_seed})')
plt.savefig('DNN_regression_mfplec_tuned_PR_curve.png', dpi=200)
print("Saved DNN_regression_mfplec_tuned_PR_curve.png and dnn_regression_mfplec_tuned_median_predictions.npy")
